...
`;
	...
	$: processedContent = renderWikiLinks(note.contentHtml, note.resolvedLinks);
	...
	async function handleCheckboxChange(index: number, isChecked: boolean) {
		...
		let newContent = note.contentHtml;
		...
		if (newContent !== note.contentHtml) {
			newContent = normalizeContentForStorage(newContent);

			const oldContent = note.contentHtml;
			note.contentHtml = newContent; // Optimistic update
			try {
				const response = await fetch(`/api/notes/${note.id}`, {
					method: 'PUT',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ contentHtml: newContent })
				});
				if (!response.ok) {
					console.error('Failed to update check status');
					note.contentHtml = oldContent; // Revert on failure
				} else {
