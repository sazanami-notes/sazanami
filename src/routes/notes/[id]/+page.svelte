<script lang="ts">
	...
	async function resolveWikiLinks() {
		...
		for (const span of Array.from(wikiLinkSpans)) {
			...
			try {
				// バックエンドAPIを呼び出し、リンクテキストからノートのIDを解決
				const response = await fetch(`/api/notes/resolve-link?title=${encodeURIComponent(linkText)}`);
				if (response.ok) {
					const { id }: Pick<ResolvedLink, 'id'> = await response.json(); // ResolvedLink から id を取得
					const url = `/notes/${id}`;
		...
		try {
			if (isNewNote) {
				...
				if (response.ok) {
					...
					// スラッグを廃止したため、リダイレクトは行わない。
					// ページはIDベースのURLにいるため、更新後のURLは変わらない。
				} else {
					const errorData: { message?: string } = await response.json();
				...
				{#if tags.length > 0 || tagsInput}
					<div class="mb-4">
						<div class="flex flex-wrap gap-2">
							<!-- タグ表示部分。タグはreadonlyでなく、tagsInputにバインドされて編集可能 -->
							<input
								type="text"
