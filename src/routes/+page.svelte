<script lang="ts">
  ...
  $: {
    ...
    const filtered = notesStore.filter((note: Note) => {
      ...
      const matchesContent = note.content ? note.content.toLowerCase().includes(searchQuery.toLowerCase()) : false;
...
<div class="container mx-auto px-4 py-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {#each filteredNotes as note}
      <MemoCard {note} username={get(page).data.user?.name} on:click={() => {
        const username = get(page).data.user?.name;
