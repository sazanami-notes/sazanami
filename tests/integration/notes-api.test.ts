...
describe('POST /api/notes', () => {
  ...
  it('should return 401 if unauthorized', async () => {
    ...
    const params = await createMockRequestHandlerParams('http://localhost/api/notes', 'POST', null, { title: 'Test', content: 'Test' });
  ...
  it('should create a note and save the correct slug', async () => {
    ...
    const params = await createMockRequestHandlerParams('http://localhost/api/notes', 'POST', mockSession, { title: noteTitle, content: 'Some content' });
    const response = await createNote(params);
    ...
    // データベースから取得して確認
    const fetchedNote = await db.select().from(notes).where(eq(notes.id, newNote.id)).limit(1).get();
  ...
  it('should create a note with japanese title and save the correct slug', async () => {
    ...
    const params = await createMockRequestHandlerParams('http://localhost/api/notes', 'POST', mockSession, { title: noteTitle, content: 'Some content in Japanese' });
    const response = await createNote(params);
    expect(response.status).toBe(201);
    const newNote: NoteResponse = await response.json();
    expect(newNote.title).toBe(noteTitle);
...
describe('PUT /api/notes/{id}', () => {
  ...
  it('should return 401 if unauthorized', async () => {
    vi.spyOn(authModule.auth.api, 'getSession').mockResolvedValueOnce(null);
    const params = await createMockRequestHandlerParams(`http://localhost/api/notes/${existingNoteId}`, 'PUT', null, { title: 'Updated Title' }, { id: existingNoteId });
  ...
  it('should return 400 if note ID is missing in params', async () => {
    vi.spyOn(authModule.auth.api, 'getSession').mockResolvedValueOnce(mockSession);
    const params = await createMockRequestHandlerParams('http://localhost/api/notes/', 'PUT', mockSession, { title: 'Updated Title' });
    const response = await updateNote(params);
    expect(response.status).toBe(400);
  ...
  it('should return 404 if note not found', async () => {
    ...
    const params = await createMockRequestHandlerParams(`http://localhost/api/notes/${nonExistentId}`, 'PUT', mockSession, { title: 'Updated Title' }, { id: nonExistentId });
    const response = await updateNote(params);
  ...
  it('should update a note and re-generate the correct slug', async () => {
    ...
    const params = await createMockRequestHandlerParams(`http://localhost/api/notes/${existingNoteId}`, 'PUT', mockSession, { title: updatedTitle, content: 'Updated content' }, { id: existingNoteId });
    const response = await updateNote(params);
    ...
    // データベースから取得して確認
    const fetchedNote = await db.select().from(notes).where(eq(notes.id, existingNoteId)).limit(1).get();
  ...
  it('should update a note with japanese title and re-generate the correct slug', async () => {
    ...
    const params = await createMockRequestHandlerParams(`http://localhost/api/notes/${existingNoteId}`, 'PUT', mockSession, { title: updatedTitle, content: 'Updated content in Japanese' }, { id: existingNoteId });
    const response = await updateNote(params);
    expect(response.status).toBe(200);
    const updatedNote: NoteResponse = await response.json();
    expect(updatedNote.title).toBe(updatedTitle);
