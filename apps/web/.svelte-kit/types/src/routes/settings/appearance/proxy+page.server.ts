...
export const actions = {
	saveSettings: async ({ request }: import('./$types').RequestEvent) => {
		const session = await auth.api.getSession({
			headers: request.headers
		});
	...
	createTheme: async ({ request }: import('./$types').RequestEvent) => {
		const session = await auth.api.getSession({
			headers: request.headers
		});
	...
	deleteTheme: async ({ request }: import('./$types').RequestEvent) => {
		const session = await auth.api.getSession({
			headers: request.headers
		});
