...
export const actions = {
	resetPasswordDirect: async ({ request }: import('./$types').RequestEvent) => {
		const sessionData = await auth.api.getSession({
			headers: request.headers
		});
	...
	changePassword: async ({ request }: import('./$types').RequestEvent) => {
	...
	unlinkAccount: async ({ request }: import('./$types').RequestEvent) => {
		const sessionData = await auth.api.getSession({
			headers: request.headers
		});
