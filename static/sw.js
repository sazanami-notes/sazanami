    ...
    if (requestUrl.pathname.startsWith('/api/')) {
        event.respondWith(
            ...
            fetch(event.request).then((response) => {
                ...
                console.log(`[Service Worker] Network failed, trying API cache for: ${requestUrl.pathname}`);
                return caches.match(event.request).then((cachedResponse) => {
                    ...
                    console.warn(`[Service Worker] Not found in API cache, returning offline fallback for: ${requestUrl.pathname}`);
                    return new Response('API is offline', { status: 503, statusText: 'Service Unavailable' });
                });
            })
        );
    } else {
        ...
        event.respondWith(
            ...
            caches.match(event.request).then((cachedResponse) => {
                ...
                console.log(`[Service Worker] Not found in cache, fetching from network: ${requestUrl.pathname}`);
                return fetch(event.request).then((response) => {
                    ...
                    console.warn(`[Service Worker] Network failed and not in cache, returning offline fallback for: ${requestUrl.pathname}`);
